...
class Book {
    ...
    get issuedCopies() { return this.#issuedCopies; }
    get availableCopies() { return this.#totalCopies - this.#issuedCopies; }
...
class Admin extends User {
    ...
    getPermissions() {
        return [
            "addBook", "delBook", "findBook", "issueBook", "returnBook",
            "addUser", "delUser", "getUsers", "getBooks"
        ];
    }
}
...
class Library {
    ...
    createAdmin(surname, name, cardNumber) {
        try {
            const existingUser = this.findUserByCard(cardNumber);
            if (existingUser) {
                throw new Error(`Пользователь с номером карточки ${cardNumber} уже существует.`);
            ...
            const admin = new Admin(surname, name, cardNumber);
            this.#users.push(admin);
            this.saveData();
            console.log(`Администратор ${admin.fullName} создан.`);
        } catch (error) {
            console.error("Ошибка createAdmin:", error.message);
        }
    }
...
function main() {
    ...
    switch (command) {
        ...
        case "userBooks":
            ...
            break;
        case "createAdmin":
            library.createAdmin(args[0], args[1], args[2]);
            break;
        case "help":
        default:
            printHelp();
